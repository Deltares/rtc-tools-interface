...
### Active constraints and shadow prices by priority
In addition to the per-goal performance metrics, the `GoalGeneratorMixin` also stores two priority-level summaries:

- `get_active_constraint_metrics()`: counts of active hard constraints, their fraction of the total, and the number of active constraints imposed by earlier priorities.
- `get_shadow_price_metrics()`: a lower-triangular matrix with, for each solved priority, the **sum of absolute shadow prices** of hard constraints introduced by earlier priorities.

For `get_shadow_price_metrics()`, the rows are the current optimization priorities (plus `final_results`) and the columns are the earlier priorities that introduced hard constraints. This means:

- the first priority has no shadow prices yet,
- priority 2 reports shadow prices from priority 1,
- priority 3 reports shadow prices from priorities 1 and 2,
- and so on.

After `optimize()`, both summaries are also written to the `output/performance_metrics/` folder:

- `active_constraint_metrics.csv`
- `shadow_price_metrics.csv`

The interactive dashboard generated by `get_performance_metrics_with_plot()` includes matching tabs for these summaries.

Shadow prices are computed from the solver's constraint multipliers. They are therefore only available when the solver returns finite dual values. In mixed-integer optimization problems, shadow prices are typically not well-defined, so `get_shadow_price_metrics()` and `shadow_price_metrics.csv` may be empty.

### Finite-difference RHS sensitivity analysis for mixed-integer problems
For mixed-integer problems, a shadow price is typically not available. To still provide insight into how restrictive hard constraints from previous priorities are, the `GoalGeneratorMixin` also offers a separate finite-difference workflow:
...
For every relaxation level, the method returns a lower-triangular matrix in which:
...
- `finite_difference_rhs_sensitivity_1pct.csv`
- `finite_difference_rhs_sensitivity_2pct.csv`
- `finite_difference_rhs_sensitivity_5pct.csv`
- `finite_difference_rhs_sensitivity_10pct.csv`
