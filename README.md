...
The equations for the range_rate_of_change goal are almost the same as for the range goal, which can be found above. The only difference is that $x_t$ is replaced by the derivative of $x$ by $t$, calculated as $\frac{x_t - x_{t-1}}{\Delta t}$.  
