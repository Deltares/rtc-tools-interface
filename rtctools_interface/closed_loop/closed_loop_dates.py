...
def read_closed_loop_dates(file_path: Path):
    ...
    for i in range(1, len(closed_loop_dates)):
        if closed_loop_dates["start_date"].iloc[i] > closed_loop_dates["end_date"].iloc[i - 1]:
            raise ValueError(f"Start date at row {i} is later than the previous end date. ")
    if any(closed_loop_dates["start_date"] < closed_loop_dates["start_date"].shift(1)):
        raise ValueError("The start dates are not in ascending order.")
    if any(closed_loop_dates["end_date"] < closed_loop_dates["end_date"].shift(1)):
        raise ValueError("The end dates are not in ascending order.")
    if any(closed_loop_dates["end_date"] < closed_loop_dates["start_date"]):
        raise ValueError("For one or more rows the end date is before the start date.")
    if any(closed_loop_dates["start_date"] > closed_loop_dates["end_date"]):
        raise ValueError("For one or more rows the start date is after the end date.")
    if (
        ...
        raise ValueError("Currently, the date ranges can only be specific up to the level of days.")
