...
from rtctools_interface.plotting.plot_tools import (
    create_plot_each_priority,
...
class PlotMixin(PlottingBaseMixin, StatisticsMixin):
    ...
    def priority_completed(self, priority: int) -> None:
        ...
        timeseries_data = self.collect_timeseries_data(
            list(set(self.custom_variables + self.state_variables))
        )
    ...
    def post(self):
        ...
        if self.solver_stats["success"]:
            base_goals = [
                goal.get_goal_config()
                for goal in self.goals() + self.path_goals()
            ...
            if self.plot_results_each_priority:
                plot_data = plot_data | create_plot_each_priority(
