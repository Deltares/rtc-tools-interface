...
class PlotMixin(PlottingBaseMixin):
    ...
    def manual_extraction_from_state_vector(self):
        for variable in self.custom_variables:
            try:
                ...
                logger.debug(f"Variable {variable} not found in output of model.")
    ...
    def post(self):
        ...
        for variable in self.custom_variables:
            if (
                not self._manual_extracted_states[variable]
                or len(self._manual_extracted_states[variable]) == 0
            ):
    ...
    def collect_timeseries_data(self, all_variables_to_store: list[str]) -> dict[str, np.ndarray]:
        return {
            variable: np.array(self._manual_extracted_states[variable])
            for variable in all_variables_to_store
        }
