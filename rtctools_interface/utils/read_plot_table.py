...
def read_plot_config_from_csv(plot_table_file: Path | str) -> list[PlotTableRow]:
...
def read_plot_config_from_list(plot_config: list[PlotTableRow]) -> list[PlotTableRow]:
    ...
    for plot_table_row in plot_config:
        if not isinstance(plot_table_row, PlotTableRow):
            raise TypeError(
                "Each element in the passed plot table should be of type 'PlotTableRow'"
            )
    return plot_config


def get_plot_config(
    plot_table_file=None, plot_config_list=None, read_from="csv_table"
) -> list[PlotTableRow]:
