...
def compare_xml_file(file_result: Path, file_ref: Path):
    ...
    for serie_result, serie_ref in zip(series_result, series_ref, strict=False):
        for event_result, event_ref in zip(
            serie_result.findall("pi:event", ns), serie_ref.findall("pi:event", ns), strict=False
        ):
            value_result = float(event_result.attrib["value"])
            value_ref = float(event_ref.attrib["value"])
            assert math.isclose(value_result, value_ref, rel_tol=R_TOL, abs_tol=A_TOL), (
...
class TestClosedLoop(TestCase):
    ...
    def compare_csv_files(self, output_folder: Path, reference_folder: Path, n_periods: int):
        ...
        self.assertTrue(output_folder.exists(), "Output modelling period folder should be created.")
        self.assertEqual(
    ...
    def test_running_closed_loop_csv(self):
        ...
        config = ClosedLoopConfig(
            file=base_folder / "input" / "closed_loop_dates.csv", round_to_dates=True
        )
    ...
    def test_running_closed_loop_csv_fixed_periods(self):
        ...
        config = ClosedLoopConfig.from_fixed_periods(
            optimization_period=timedelta(days=3), forecast_timestep=timedelta(days=2)
        )
        run_optimization_problem_closed_loop(
            ExampleCsv, base_folder=base_folder, config=config, output_folder=output_folder
        )
        self.compare_csv_file(
